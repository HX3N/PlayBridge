// Fake `external_renderer_ipc.dll` — exports the nemu ABI that MAA's MumuExtras path loads,
// serving frames from PlayBridge's WGC daemon instead of an emulator framebuffer.
//
// Deploy: rename the built DLL to <fakemumu>/nx_device/15.0/shell/sdk/external_renderer_ipc.dll
...
// MumuExtras is only used for screencap.
#[no_mangle]
