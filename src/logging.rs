...
// Resolved once per process. The WGC daemon logs on every frame request, so re-deriving the exe path
// and re-creating the directory per line cost two syscalls each time.
